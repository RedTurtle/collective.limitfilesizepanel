    ...
    def check_file_size(self, metatypes, request):
        ...
        try:
            helper_view = api.content.get_view(
                name="lfsp_helpers_view", context=api.portal.get(), request=request,
            )
        ...
        if not maxsize:
            return None
        return helper_view.check_size(maxsize=maxsize, uploadfile=request["uploadfile"])
